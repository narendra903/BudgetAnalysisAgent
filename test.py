...
async def initialize_knowledge_bases():
    ...
    for pdf_file in pdf_files:
         st.write(f"Checking file: {pdf_file}")  # Debug: Show the path being checked
    st.write(f"Exists: {pdf_file.exists()}")  # Debug: Check if file exists
    st.write(f"Resolved path: {pdf_file.resolve()}")  # Debug: Show absolute path
    if pdf_file.exists():
        try:
